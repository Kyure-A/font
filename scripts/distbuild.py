...
def build_files(version: Version) -> None:
    ...
    font_paths = [
        dist_dir.joinpath(f"kyre-font-Regular.{ext}") for ext in ["ttf", "woff"]
    ]
    font = fontbuild.build(version=version.get_font_version())
    for path in font_paths:
        fontbuild.generate_file(font=font, path=path)

    with zipfile.ZipFile(dist_dir.joinpath("kyre-font.zip"), "w") as zipf:
...
def build_npm_readme_content() -> str:
    ...
    try:
        ...
        git_tag = None
    if git_tag:
        base_url = "https://raw.githubusercontent.com/Kyure-A/font"
...
def build_package_json_content(*, version: Version) -> str:
    metadata = {
        ...
        "repository": "https://github.com/Kyure-A/font",
        "author": "Kyure_A",
        "license": "OFL-1.1",
    }
